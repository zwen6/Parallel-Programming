...
int initMatrix(const char *fname) {
  ...
  auto at = fgets(buffer, 1024, file);
...
//
void computeGauss(int nsize, int thread_num, std::string mode) {
  int i, j, k;
  double pivotval;
  if(mode == "blocked"){
  for (i = 0; i < nsize; i++) {
    ...
    #pragma omp parallel for private(j,k,pivotval) num_threads(thread_num) schedule(static, nsize/thread_num)
    for (j = i + 1; j < nsize; j++) {
      pivotval = matrix[j][i];
      matrix[j][i] = 0.0;
      for (k = i + 1; k < nsize; k++) {
  ...
  for (i = 0; i < nsize; i++) {
    ...
    // Factorize the rest of the matrix:
    #pragma omp parallel for private(j,k,pivotval) num_threads(thread_num) schedule(static, 1)	
    for (j = i + 1; j < nsize; j++) {
      pivotval = matrix[j][i];
      matrix[j][i] = 0.0;
      for (k = i + 1; k < nsize; k++) {
  ...
  for (i = 0; i < nsize; i++) {
    ...
    // Factorize the rest of the matrix:
    #pragma omp parallel for private(j,k,pivotval) num_threads(thread_num) schedule(dynamic)	
    for (j = i + 1; j < nsize; j++) {
      pivotval = matrix[j][i];
      matrix[j][i] = 0.0;
      for (k = i + 1; k < nsize; k++) {
...
int main(int argc, char *argv[]) {
  ...
  for (int ind = 1; ind < argc-1; ind++){
      if(std::string(argv[ind])=="-t"){
          thread_num = std::stol(argv[ind+1]);
      }
      else if(std::string(argv[ind])=="-m"){
          mode = argv[ind+1];
      }
  }
  ...
  computeGauss(nsize, thread_num, mode);
